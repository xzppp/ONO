...
class ONOBlock(nn.Module):
    ...
    def forward(self, input: torch.Tensor , fx):
        ...
        ort_x = orthogonal_tensor(x)
        fx = torch.matmul(torch.matmul(ort_x,ort_x.transpose(-2,-1)),fx)
        fx = self.act(fx)
...
class ONO(nn.Module):
    def __init__(self,
        ...
        self.blocks = nn.Sequential(*[ONOBlock(num_heads = n_head, hidden_dim= n_hidden, dropout= ffn_dropout, attention_dropout= attn_dropout) for _ in range(n_layers)])
